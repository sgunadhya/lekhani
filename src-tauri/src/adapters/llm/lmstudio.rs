...
#[derive(Clone)]
pub struct LmStudioNarrativeEngine {
    client: Client,
    base_url: String,
    model: Option<String>,
}

impl LmStudioNarrativeEngine {
    pub fn new_from_env() -> Result<Self, String> {
        if std::env::var("LEKHANI_LLM_PROVIDER")
            .ok()
            .as_deref()
            != Some("lmstudio")
        {
            return Err("lmstudio not selected".to_string());
        }

        let timeout_seconds = std::env::var("LEKHANI_LMSTUDIO_TIMEOUT_SECS")
            .ok()
        ...
        let base_url = std::env::var("LEKHANI_LMSTUDIO_BASE_URL")
            .unwrap_or_else(|_| "http://127.0.0.1:1234/v1".to_string())
            .trim_end_matches('/')
            .to_string();
        let model = std::env::var("LEKHANI_LMSTUDIO_MODEL").ok();
    ...
    fn resolve_model(&self) -> Result<String, String> {
        ...
        let payload: ModelsResponse = response.json().map_err(|err| err.to_string())?;
        payload
            .data
            .into_iter()
            .next()
            .map(|item| item.id)
            .ok_or_else(|| "LM Studio returned no loaded models".to_string())
    }
    ...
    fn respond_json(&self, label: &str, system: &str, user: &str) -> Result<Value, String> {
        ...
        match self.request_completion_content(&payload) {
            Ok(content) => {
            ...
            Err(first_error) => {
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": format!(
                                "{}\nReturn only a single valid JSON object and no surrounding commentary.",
                                system
                            )
                        },
                        { "role": "user", "content": user }
                ...
                trace_block(label, "retry_reason", &first_error);
                let content = self
                    .request_completion_content(&fallback_payload)
                    .map_err(|second_error| {
                        format!(
                            "LM Studio request failed. First attempt: {}. Retry without response_format: {}",
                            first_error, second_error
                        )
                    })?;
                trace_block(label, "response_raw_retry", &content);
                let parsed = parse_single_json_object(&content)?;
                trace_block(label, "response_json_retry", &parsed.to_string());
                Ok(parsed)
            }
        }
    }
    ...
    fn request_completion_content(&self, payload: &Value) -> Result<String, String> {
        ...
        if !status.is_success() {
            ...
            return Err(format!("HTTP {} from LM Studio: {}", status, detail));
        }
        let completion: ChatCompletionsResponse =
            serde_json::from_str(&body).map_err(|err| format!("Invalid LM Studio response JSON: {err}. Body: {}", body.trim()))?;
        completion
            .choices
            .into_iter()
            .next()
            .and_then(|choice| choice.message.content)
            .ok_or_else(|| "LM Studio returned no completion content".to_string())
    }
}
...
impl AssistantAgent for LmStudioNarrativeEngine {
...
impl CharacterParser for LmStudioNarrativeEngine {
    fn parse_character(
        ...
        Ok(NarrativeCharacter {
            ...
            name: value["name"].as_str().unwrap_or("Unnamed Character").trim().to_string(),
...
impl EventParser for LmStudioNarrativeEngine {
    fn parse_event(
        &self,
        description: &str,
        snapshot: &NarrativeSnapshot,
        ...
        Ok(NarrativeEvent {
            id: Uuid::new_v4(),
            ontology_entity_id: None,
            title: value["title"].as_str().unwrap_or("Untitled Event").trim().to_string(),
            summary: value["summary"].as_str().unwrap_or(description).trim().to_string(),
    ...
    if let Some(start) = text.find('{') {
        if let Some(end) = text.rfind('}') {
            return serde_json::from_str(&text[start..=end])
                .map_err(|err| format!("LM Studio returned invalid JSON: {err}"));
        }
    }
    Err("LM Studio did not return a JSON object".to_string())
}
...
fn parse_focus_reply(value: Value, fallback_focus: &str) -> Result<FocusReply, String> {
    ...
    let body = value
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "LM Studio reply is missing body".to_string())?
        .to_string();
